...
export type Error = ErrorBadStatus | ErrorNotUtf8 | ErrorIo;
export interface ErrorBadStatus {
  tag: "BadStatus",
  val: string,
}
export interface ErrorNotUtf8 {
  tag: "NotUtf8",
  val: string,
}
...
export class Tarball {
  ...
  drop(): void;
  static fetch(exports: Exports, url: string): Promise<Result<Tarball, Error>>;
  files(): string[];
  contents(idx: number): string;
}
